...
import cn.hx.kefu.recorder.AudioManager;
import cn.hx.kefu.recorder.DialogManager;
...
public class RecorderButton extends AppCompatButton implements AudioManager.AudioStateListener {
    ...
    private static int DISTANCE_Y_CANCEL = 50;
    ...
    public RecorderButton(Context context, AttributeSet attrs) {
        super(context, attrs);
//        String dir = Environment.getExternalStorageDirectory() + "/easemob_recorder_audios/";
        File voicePath = getVoiceFilePath(context);
        if (!voicePath.exists()) {
            voicePath.mkdirs();
        }

        String dir = voicePath.getPath();
        Log.i("info", dir);
...
//                isRecording = true;
    ...
    @NonNull
    private File getVoiceFilePath(Context context) {
        String appPackageName = context.getPackageName();
        String pathPrefix = "/Android/data/" + appPackageName + "/";
        String loginUsername = SLoginConfig.getLoginUsername();
        String path = pathPrefix + "/" +(loginUsername==null?"agent":loginUsername)  + "/video/";
        File storageDir;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_VOICE_CHANGE:
                    if (null != button) {
                        button.mDialogManager.updateVoiceLevel(button.mAudioManager.getVoiceLevel(7));
                    }
